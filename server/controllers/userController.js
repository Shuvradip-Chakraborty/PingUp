...
export const updateUserData = async (req, res) => {
  try {
    ...
    //if profile image is available then convert it into base 64 image and upload it and generate the url
    if (profile) {
        //may things had changed regarding imagekit so old method is in the comment block..
    ...
    //if cover image is available then convert it into base 64 image and upload it and generate the url
    if (cover) {
...
export const followUser = async (req, res) => {
  try {
    ...
    //this is for the other user whom i want to follow
...
export const unfollowUser = async (req, res) => {
  try {
    const { userId } = req.auth();
    const { id } = req.body; //this is for the another useer..
...
export const sendConnectionRequest = async (req, res) => {
    try {
        const {userId} = req.auth() //id of logged in user
        const {id} = req.body; //id of other user...
...
export const acceptConnectionRequest = async (req, res) => {
  try {
    const { userId } = req.auth(); //id of logged in user
    const {id} = req.body;  //id of another user.
